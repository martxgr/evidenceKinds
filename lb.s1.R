...
pca <- prcomp(d.cluster, scale. = TRUE)
d_plot <- data.frame(PC1 = pca$x[,1], PC2 = pca$x[,2],
                     cluster = factor(mod1$classification))
ggplot(d_plot, aes(PC1, PC2, color = cluster)) +
  geom_point(alpha = 0.6) +
  stat_ellipse(level = 0.95) +
  theme_minimal()

means <- data.frame(t(mod1$parameters$mean))
means$cluster <- factor(1:2)
means_long <- pivot_longer(means, -cluster, names_to = "item", values_to = "mean")
ggplot(means_long, aes(x = item, y = mean, color = cluster, group = cluster)) +
  geom_point(size = 3) + geom_line(linewidth = 1) +
  theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1))

posteriors <- mod1$z
max_post <- apply(posteriors, 1, max)
hist(max_post, breaks = 30, main = "Classification certainty")
...
sim_results <- parLapply(cl, 1:n_sims, function(i) {
  ...
  # Fit 2-class GMM
  gmm_sim <- Mclust(X_sim, G = 2, modelNames = "VVV", verbose = FALSE)
...
p2 <- ggplot(d.complete, aes(really))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p3 <- ggplot(d.complete, aes(consider))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p4 <- ggplot(d.complete, aes(predict))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p5 <- ggplot(d.complete, aes(context))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p6 <- ggplot(d.complete, aes(surprise))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p7 <- ggplot(d.complete, aes(bet))+
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p8 <- ggplot(d.complete, aes(jus)) +
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)

p9 <- ggplot(d.complete, aes(double)) +
  geom_histogram(aes(y = ..density..), bins = 5, fill = "skyblue", alpha = 0.7, boundary = 0) +
  geom_density(color = "salmon", linewidth = 0.5) +
  theme_minimal()+
  facet_wrap(~domain)
...
# Get factor scores
factor_scores <- predict(fit_cfa)
domain_visualization <- d.complete %>%
  mutate(factor_score = factor_scores[,1])

ggplot(domain_visualization, aes(x = factor_score, fill = domain)) +
  geom_histogram(bins = 8, alpha = 0.6, position = "stack") +
  theme_minimal() +
  labs(x = "Factor Score", y = "Count", title = "Distribution of Factor Scores by Domain")
